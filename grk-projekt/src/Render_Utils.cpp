...
void Core::RenderContext::initFromArray(std::vector<glm::vec3> vectors, std::vector<glm::uvec3> indices, std::vector<glm::vec3> normals) {
    ...
    unsigned int vertexDataBufferSize = sizeof(float) * vectors.size() * 3;
    unsigned int vertexNormalBufferSize = sizeof(float) * normals.size() * 3;

    unsigned int vertexElementBufferSize = sizeof(unsigned int) * indices.size() * 3;
    size = indices.size() * 3;
    ...
    glBufferData(GL_ARRAY_BUFFER, vertexDataBufferSize + vertexNormalBufferSize, NULL, GL_STATIC_DRAW);

    glBufferSubData(GL_ARRAY_BUFFER, 0, vertexDataBufferSize, &vectors[0]);
